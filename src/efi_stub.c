...
EFI_STATUS efi_main(EFI_HANDLE img_handle, EFI_SYSTEM_TABLE* st) {
    ...
    char* kernel_loader_region;
    {
        ...
        status = st->BootServices->OpenProtocol(img_handle, &loaded_img_proto_guid, (void**)&loaded_img_proto,
												img_handle, NULL, EFI_OPEN_PROTOCOL_GET_PROTOCOL);
        if (status != 0) {
            printhex(st, status);
        ...
        status = st->BootServices->OpenProtocol(dev_handle, &simple_fs_proto_guid, (void**)&simple_fs_proto, img_handle,
    ...
    // Get linear framebuffer
    {
        ...
        status =
            st->BootServices->LocateProtocol(&graphics_output_protocol_guid, NULL, (void**)&graphics_output_protocol);
    ...
    //   details so the memory map, framebuffer, the page tables themselves
    {
        ...
        Elf64_Shdr* string_table =
            (Elf64_Shdr*)&elf_file[elf_header->e_shoff + (elf_header->e_shstrndx * elf_header->e_shentsize)];
        if (string_table->sh_type != SHT_STRTAB) { panic(st, L"No string table found in kernel.o"); }
        ...
        for (size_t i = 0; i < section_count; i++) {
            ...
            if (section->sh_flags & SHF_ALLOC) { pages_necessary += (section->sh_size + (PAGE_SIZE - 1)) / PAGE_SIZE; }
        }
		
        // Allocate said space plus enough space for page tables
        // Framebuffer is also kept so we wanna make some pages for it
        size_t page_tables_necessary = estimate_page_table_count(pages_necessary) +
        ...
        EFI_PHYSICAL_ADDRESS addr;
        status = st->BootServices->AllocatePages(AllocateAnyPages, EfiLoaderData, pages_necessary, &addr);
        if (status != 0) {
            printhex(st, status);
            ...
            for (size_t i = 0; i < reloc_size; i += sizeof(Elf64_Rela)) {
                ...
                if (type == R_X86_64_PLT32) {
                    ...
                    *(uint32_t*)(text_section_ptr + rela->r_offset) = (uint32_t)new_address;
                    ...
                    // Compute the new address to patch, based on symbol location and addend
                    uint64_t patch_address  = (uint64_t)(text_section_ptr + rela->r_offset);
                    uint64_t symbol_address = (uint64_t)&section_table[sym->st_shndx][sym->st_value];
                    uint64_t new_address    = (symbol_address + rela->r_addend) - patch_address;
        ...
        PageTableContext ctx = { .capacity = page_tables_necessary, .used = 1, .tables = page_tables };
		
        // Identity map all the stuff we wanna keep
        if (identity_map_some_pages(st, &ctx, (uint64_t)section_memory, pages_necessary)) { return 1; }
		
        uint64_t fb_page_count = PAGE_4K(boot_info.fb.width * boot_info.fb.height * 4);
        if (identity_map_some_pages(st, &ctx, (uint64_t)boot_info.fb.pixels, fb_page_count)) { return 1; }
		
        if (identity_map_some_pages(st, &ctx, (uint64_t)&mem_map_buffer[0], PAGE_4K(MEM_MAP_BUFFER_SIZE))) { return 1; }
		
        if (identity_map_some_pages(st, &ctx, (uint64_t)&kernel_loader_region[0], PAGE_4K(LOADER_BUFFER_SIZE))) {
            return 1;
        }
		
        if (identity_map_some_pages(st, &ctx, (uint64_t)&kernel_stack[0], PAGE_4K(KERNEL_STACK_SIZE))) { return 1; }
