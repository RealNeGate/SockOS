...
local efi_cflags = "-target x86_64-pc-win32-coff -fno-PIC -fshort-wchar -I kernel "..cflags
...
do
    ...
    table.insert(lines, "flags = -c -fPIC -fno-omit-frame-pointer -I ext -I kernel -DKERNEL_LAND "..elf_cflags)

    local objs = {}
    local asm_files = list_files("arch/x64/", "*.s")
    ...
    local arch_files = list_files("arch/x64/", "*.c")
    ...
    local files = list_files("kernel/", "*.c")
    ...
    build("objs/kernel.so | objs/kernel.map", "ld", table.concat(objs, " ").." | kernel/link.ld", { flags="-T kernel/link.ld -Map=objs/kernel.map -nostdlib -pie" })
    table.insert(lines, "")
end
...
do
    ...
    build("objs/efi.o", "cc", "boot/efi_main.c | objs/kernel.so objs/kernel.map objs/initrd", {
        flags = efi_cflags.." -c"
    })
